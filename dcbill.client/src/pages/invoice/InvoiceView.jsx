...
import {
    ...
    Grid, Divider, Table, TableBody, TableCell,
...
const sx = {
    ...
    // The invoice card — styled like a document
    invoiceCard: {
        bgcolor: T.card,
        border: `1px solid ${T.border}`,
        borderRadius: '16px',
    ...
    // Section card inside the doc body
    sectionCard: {
        border: `1px solid ${T.border}`,
        borderRadius: '10px',
        p: 2.5,
...
function MetaLabel({ icon: Icon, label, value, light }) {
    return (
        <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.3 }}>
                <Icon sx={{ fontSize: '0.85rem', color: light ? 'rgba(255,255,255,0.55)' : T.faint }} />
                <Typography sx={{ fontSize: '0.7rem', color: light ? 'rgba(255,255,255,0.55)' : T.faint, fontWeight: 500, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
                    {label}
                </Typography>
            </Box>
            <Typography sx={{ fontSize: '0.9rem', fontWeight: 600, color: light ? '#fff' : T.text }}>
                {value || '—'}
            </Typography>
        </Box>
    );
...
export default function InvoiceView() {
    ...
    const {
        ...
        cgstAmount, sgstAmount, cgstPercent, sgstPercent,
    ...
    const preparePdfData = async () => {
        ...
        return {
            ...
            subtotal, totalGST, grandTotal, gstPercent,
            cgstPercent, sgstPercent, cgstAmount, sgstAmount,
        };
    };
    ...
    if (loading || billingLoading) {
        return (
            <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100vh', gap: 2, bgcolor: T.bg }}>
                ...
                <Typography sx={{ color: T.muted, fontSize: '0.85rem' }}>Loading invoice…</Typography>
            </Box>
        );
    }
    ...
    if (!currentInvoice) {
        return (
            <Box sx={{ p: 4, textAlign: 'center', bgcolor: T.bg, minHeight: '100vh' }}>
                ...
                <Typography sx={{ color: T.muted, mb: 3, fontSize: '0.85rem' }}>The invoice you're looking for doesn't exist or was removed.</Typography>
                    ...
                    <Box sx={sx.docHeader}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: { xs: 'flex-start', sm: 'flex-end' }, gap: 1.5 }}>
                            <Box sx={sx.pill}>
                                ...
                                <Typography sx={{ fontSize: '0.82rem', fontWeight: 700, color: '#fff', letterSpacing: '0.02em' }}>
                            ...
                            <Box sx={{ ...sx.sectionCard, display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                                <Box sx={{ flex: '1 1 200px' }}>
                                    ...
                                    <Typography sx={{ fontSize: '0.82rem', color: T.muted }}>
                                        {[currentInvoice.partyCity, currentInvoice.partyPinCode, currentInvoice.partyState]
                                            .filter(Boolean).join(' – ')}
                                    </Typography>
                                </Box>
                                {currentInvoice.partyGSTIN && (
                                    <Box sx={{ flex: '0 0 auto' }}>
                                        <Typography sx={{ fontSize: '0.7rem', color: T.faint, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', mb: 0.3 }}>
                                            GSTIN
                                        </Typography>
